...
class SimulationLocalSearch:
    def __init__(self, network, cars: list):
        self.network = network
        self.cars = cars
        self.k = 3 # Anzahl der möglichen nächsten Maschinen zur Auswahl
    ...
    def print_leerfahrten(self):
        for car in self.cars:
            print(f"🚗 Car {car.id}: Anzahl Leerfahrten = {car.num_leerfahrten}")
        print("Insgesamte Leerfahrten:", sum(car.num_leerfahrten for car in self.cars))
    ...
    def find_next_machine_randomly(self, start_machine):
        ...
        if not start_demand.empty:
            ...
            # 🔀 Sortieren nach Distanz und dann zufällig einen aus den Top-3 wählen
            start_demand = start_demand.sort_values("distance")
            candidates = start_demand.head(self.k)

            # zufällig eine Zeile auswählen
            best_row = candidates.sample(1).iloc[0]

            return int(best_row["dest"])

        # 2️⃣ Falls keine Aufträge am Standort → nächste Startmaschine suchen
        ...
        # 🔀 Top-3 Maschinen nach Distanz nehmen
        sorted_machines = sorted(distances.items(), key=lambda x: x[1])
        k = min(3, len(sorted_machines))
        candidates = sorted_machines[:k]

        # zufällige Maschine auswählen (nur Namen extrahieren)
        nearest_machine = random.choice(candidates)[0]
